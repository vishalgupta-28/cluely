...
def handle_help_from_ai(custom_instructions, temperature, top_p, token_limit, raw_conversation, meetingTemplate=""):
...
def handle_help_from_ai_text(custom_instructions, temperature, top_p, token_limit, raw_conversation, highlightedText, meetingTemplate=""):
    log_time("Starting Question Extraction")
...
def llm_processing_query(context_text, query, custom_instructions, temperature, top_p, token_limit, raw_conversation, meetingTemplate=""):
    """
    ...
    Args:
        ...
        raw_conversation (list): User's conversation history
    ...
    # Process conversation into summarized text similar to handle_help_from_ai
    relevant_conversation = extract_relevant_conversation(raw_conversation)
    summarized_text = " ".join(f'{sender}:{text}' for sender,text in relevant_conversation) if relevant_conversation else "No new conversation available to analyze."
...
f"""You are an assistant for question-answering tasks. 
                    ...
                    Analyze everything and provide a comprehensive answer. 
                    The context may include both web search results and document excerpts from a vector database.
                    If you don't know the answer, just say that you don't know. 
                    Use information from the context to provide a comprehensive answer.
                    
                    IMPORTANT: Gave response with md format , When citing information from the sources, use numbered citations in square brackets like [1], [2], [3] etc. 
                    Each source is numbered in the Sources section below. Always cite the specific source number when referencing information.
                    Gave Response in markdown format.
...
def HELP_WITH_AI(raw_conversation, use_web, userId, useRag, meetingTemplate, documents):
    try:
        ...
        query_stream = handle_help_from_ai(instruction, temperature, top_p, token_limit, raw_conversation, meetingTemplate)
        if not query_stream:
        ...
        response_generator = llm_processing_query(retrieved_docs, query, instruction2, temperature, top_p, token_limit, raw_conversation, meetingTemplate)
        ...
        log_time(f"Error in HELP_WITH_AI: {e}")
        yield json.dumps({"error": f"An error occurred during AI help processing: {str(e)}"}) + "\n"


def HELP_WITH_AI_text(raw_conversation, use_web, userId, highlightedText, meetingTemplate, useRag=False):
    ...
    try :
        ...
        # Get streaming query generator
        query_stream = handle_help_from_ai_text(instruction, temperature, top_p, token_limit, raw_conversation, highlightedText, meetingTemplate)
        if not query_stream:
            yield json.dumps({"error": "Failed to generate query"}) + "\n"
            return
        ...
        # Get the streaming generator, now passing raw_conversation
        response_generator = llm_processing_query(context_text, query, instruction2, temperature, top_p, token_limit, raw_conversation, meetingTemplate)
